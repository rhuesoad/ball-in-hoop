function run = T2_tracking_8deg_1hz()
%T2_TRACKING_8DEG_1HZ  LQR tracks a 8 deg / 1 Hz sinusoid, outer hoop.
%   Matches the bench runs in python/analysis/npz (T2_A08.0deg_f01.00Hz).

    params = ball_hoop_params();
    [~, R_eff_o] = hoop_geometry('rolling_out', params);

    scn.id   = 'T2_tracking_8deg_1hz';
    scn.name = 'T2 - Sine 8 deg at 1 Hz (outer)';
